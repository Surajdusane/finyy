...
export function calculatePercentage(current: number, previous: number, precision: number = 2): number {
...
export function fillMisingDays(
    ...
    const transactionByDay = allDays.map((day) => {
        ...
        const found = activeDays.find((item) => normalizeDate(item.date).getTime() === normalizedDay.getTime());
        if (found) {
